...
way it does in production. ``POST /oauth/token`` always has exactly one owner,
which dispatches on ``grant_type``; the last two cases turn the device grant on
to prove the two grants share that owner. Proves the whole path:
...
- with the device grant enabled, a configured machine client still mints —
  the two grants coexist on the shared endpoint rather than racing for it;
...
def _build_app(
    ...
    *,
    ...
    :param device_grant: When True, enable the device grant, which then mounts
        the shared token endpoint and carries this grant's branch into it.
    ...
    # Device grant OFF by default, so the login-grant router mounts the shared
    # token endpoint. Turned on, the device-grant router mounts it instead —
    # either way the machine branch rides along.
    if device_grant:
    ...
    # these unset is the grant's opt-out, and builds no machine branch.
    for var in (
...
async def test_default_deployment_refuses_the_grant_type(
    unconfigured_env: httpx.AsyncClient,
) -> None:
    """BLOCKING contract: opting out must leave the grant unable to mint.

    ``POST /oauth/token`` is routed either way — the login-grant refresh owns
    it independently of any machine client — so "off" cannot be read off the
    status code alone. What must hold is that no machine handler runs: a
    correct client id and secret still get ``unsupported_grant_type``, never
    a token.
    """
...
async def test_machine_client_mints_while_the_device_grant_is_enabled(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """BLOCKING contract: the two grants coexist on one token endpoint.

    ``POST /oauth/token`` has a single owner that dispatches on ``grant_type``,
    and the device-grant router forwards the machine branch into it. Turning
    the device grant on must therefore leave machine auth working — the earlier
    design stood one grant down, which read as a healthy server that failed
    every driver run at token exchange.
    """
    from omnigent.db.utils import clear_engine_cache
...
async def test_malformed_machine_config_fails_startup_with_the_device_grant_on(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """A half-configured machine client is a startup error, device grant or not.

    The config is parsed before either router mounts, so a deploy
    that misspelled one variable fails immediately rather than coming up healthy
    and revealing the mistake at the first token exchange.
    """
    from omnigent.db.utils import clear_engine_cache
