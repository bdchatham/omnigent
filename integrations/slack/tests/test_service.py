...
async def test_setup_completion_resumes_the_pending_message(tmp_path: Path) -> None:
    ...
    # Setup saves a managed-sandbox config, then fires the completion hook that
    # app.py wires to this method.
    await _configure_user(store, "T1", "U1", agent_id="ag_9", workspace="", host_type="managed")
    await service.resume_pending_message("T1", "U1", slack)
    stream = await _wait_for_stream_stop(slack)
    await service.shutdown()

    # Ran once, with the agent and host type setup just saved.
    assert omnigent.turns == [("conv_1", "what changed?")]
    assert omnigent.created[0][0] == "ag_9"
    assert omnigent.created_host_types == ["managed"]
    assert omnigent.turn_host_types == ["managed"]
    assert omnigent.bound == []  # managed: no runner to launch
