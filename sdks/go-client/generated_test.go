...
// TestModuleShipsItsOwnLicence is L1. A nested Go module is distributed as a zip
// of the files under its own directory — verified with golang.org/x/mod/zip, which
// is what cmd/go builds one with — so the repository root's LICENSE and NOTICE do
// not travel with it. Without them the module is not redistributable and
// pkg.go.dev renders no licence, which is how it decides whether to publish
// documentation at all.
//
// Against feat/go-client-v2 this fails: sdks/go-client has no LICENSE.
func TestModuleShipsItsOwnLicence(t *testing.T) {
	...
	for _, name := range []string{"LICENSE", "NOTICE"} {
		t.Run(name, func(t *testing.T) {
			...
			// Byte-identical to the repository root's, when this is a checkout
			// rather than an extracted module zip.
