...
func TestRedirectsNeverCarryCredentialsOffTheBaseURL(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			call: func(ctx context.Context, c *Client) error {
				_, err := c.SendMessage(ctx, "conv_1", "the caller's prompt text")
				return err
			},
		},
		{
...
func TestRedirectsOnTheSameEndpointStillWork(t *testing.T) {
	...
	session, err := client.GetSession(context.Background(), "conv_1", GetSessionOptions{})
	if err != nil {
		t.Fatalf("GetSession: %v, want the same-host same-scheme redirect to be followed", err)
	}
	if session.ID != "conv_1" {
		t.Errorf("session.ID = %q, want conv_1", session.ID)
	}
	if hops != 1 {
...
func TestSendInputCannotReportADroppedWriteAsSuccess(t *testing.T) {
	...
	accepted, err := client.SendMessage(context.Background(), "conv_1", "hello")
	if err == nil {
		t.Fatalf("SendMessage = %+v, nil error: the input was dropped by a redirect and reported queued",
			accepted)
	}
	if !errors.Is(err, ErrUnsafeRedirect) {
		t.Fatalf("error = %v, want it to wrap ErrUnsafeRedirect", err)
